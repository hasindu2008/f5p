...
#folder=/mnt/778/778-5000ng/778-5000ng_albacore-2.1.3
    ...
    #minimap
    /usr/bin/time -v $MINIMAP -x map-ont -a -t4 -K10M --secondary=no  --multi-prefix=$TMP $REFIDX $FASTQLOCAL > $SAMLOCAL 2>> $LOGLOCAL || exit_status=1
    ...
    #methylation
    /usr/bin/time -v $NANOPOLISH call-methylation -t 4 -r  $FASTQLOCAL -g $REF -b $BAMLOCAL -K 512 > $METHLOCAL  2>> $LOGLOCAL || exit_status=1   

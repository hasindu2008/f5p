...
ls /scratch_nas/778_prom/fast5/* -lhS | awk '{print $9}' > data/dev.cfg;

ansible all -m copy -a "src=scripts/fast5_pipeline_nas_prom.sh dest=/nanopore/bin/fast5_pipeline.sh mode=0755"
...
mv *.cfg data/logs/
cp $0 data/logs/
cp scripts/fast5_pipeline_nas_prom.sh data/logs/